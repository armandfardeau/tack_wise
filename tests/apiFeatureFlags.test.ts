...
jest.mock('../lib/vercelFlags', () => ({
  evaluateFeatureFlags: jest.fn().mockResolvedValue({ sailBoomLength: 72, sailStrokeWidth: 5, darkSailInLightMode: true, rightInspectorPanel: true, ruleHighlightStrokeWidth: 30 }),
}));
...
describe('/api/feature-flags', () => {
  ...
  it('returns evaluated flags for GET requests', async () => {
    ...
    expect(response.json).toHaveBeenCalledWith({ sailBoomLength: 72, sailStrokeWidth: 5, darkSailInLightMode: true, rightInspectorPanel: true, ruleHighlightStrokeWidth: 30 });
