...
export function useCanvasViewport(contentBounds?: CanvasContentBounds) {
  ...
  const fitCanvasToContent = (contentRect: CanvasContentRect) => {
    ...
    const { position: nextPosition, zoom: nextZoom } = getCanvasFitTransform(contentRect, currentStageSize, {
      padding: CANVAS_FIT_PADDING,
      topInset: CANVAS_FIT_TOP_CONTROL_OFFSET,
      bottomInset: CANVAS_FIT_BOTTOM_CONTROL_OFFSET,
    });
